...
- Added a contained Boyd field-relativity footnote after checking the 1999 chapter text: Boyd's `disciplinary matrix` is treated as the declared inferential project rather than as a social field, and his claim that accommodation demands are homeostatically related is handled as a support claim this paper asks to be earned. Rebuilt successfully; PDF remains 9 pages with only the known `fancyhdr`/`microtype` warnings.
