...
2026-06-13 — Working title: "Kinds as Projectibility Profiles." Reason: gives the synthesis paper a mature-view label and avoids making homeostasis the headline criterion.

2026-06-13 — Scope: short synthetic position paper rather than another full theory paper. Reason: the contribution should be framework consolidation and citable vocabulary, not a repetition of the HPC book.

2026-06-13 — Source architecture: treat "Not every stable cluster is homeostatic" as the general correction, "Definiteness and Deitality in English" as the linguistic application, "Naturalizing Typological Kinds" as the methodological apparatus, and "Truth-Tracking Profiles" as the bridge from older HPC vocabulary to mature profile language.

2026-06-13 — Infrastructure: point the project to the central bibliography and central house style by symlink. Reason: this project should share the current house-style and central-bib surfaces rather than carrying stale local copies.

2026-06-13 — First-draft architecture: use a six-section compact synthesis structure: introduction, profile package, projectibility-first priority, cross-level portability, failure modes/demotion, and payoff. Reason: Roughdraft review confirmed that Sections 1–3 should define the view, Section 4 should show portability without new evidence burdens, Section 5 should provide the disciplinary teeth, and Section 6 should state the payoff.

2026-06-13 — Interpret "projectibility first" as a methodological and explanatory order, not a strong metaphysical grounding thesis. Reason: a short vocabulary paper can defend starting with the projection question, but a constitutive metaphysical thesis about what grounds projectibility would require a larger argument.

2026-06-13 — Treat a projectibility profile as an analytic specification of a worldly pattern. Reason: this avoids a slide between profile-as-representation and profile-as-worldly-structure while preserving the realist constraint that successful kind claims answer to available projections.
...
2026-06-17 — Cross-fertilization pointer to the companion methods paper Difference-in-Differences_for_Corpus_Linguistics (Reynolds, in progress). Its diagnostic-to-claim decision ladder for causal identification is a worked instance of this paper's support-grade-and-demotion discipline applied to causal-inference claims: "bounded estimate" = a graded supported projection; "descriptive only" = support-grade demotion (projection stands, support downgraded); "shared wave, not policy" = scope/attribution localization; "not identified" = rejection. It sits alongside the deitality/passive linguistic profiles as evidence the support-grade/demotion machinery does real work outside metaphysics. Potential use here: cite the DiD paper as an applied instance if a concrete causal-inference example would strengthen the "profiles across levels" or "failure modes and demotion" sections. No change to this manuscript yet; logged for when the DiD paper is citable. Reason: portfolio cross-fertilization recorded at both ends; the natural flow is DiD-ladder-as-instance-of-profile-analysis, more useful to this paper than the reverse is to the methods paper.

2026-06-18 — Added an intro paragraph positioning the account against Egami & Hartman's external-validity framework (\citep{egami2023elements}, APSR 2023), placed after the Ereshefsky & Reydon comparison; added Egami & Hartman to references-local.bib (verified against Cambridge Core, DOI 10.1017/S0003055422000880); added Ryan Briggs to acknowledgements. Reason: Briggs (correspondence, June 2026) named transportability/external validity as the closest statistical cousin. The paragraph marks the shared target-relativity (their generalization targets are scope conditions) and the one added axis: profile analysis perturbs the category definition itself, not just population/treatment/outcome/context. The statistical development of this lives in the sister project Metaphysics_Truth_Statistics_WIP (projection-scope estimation), which does NOT yet cite Egami & Hartman; propagate the verified entry and the "added axis" + sign-generalization framing there.
